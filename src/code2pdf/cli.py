...
def build(
    ...
    output_dir: Optional[Path] = typer.Option(None, "--output-dir", "-d", help="Output directory for generated files"),
    format: OutputFormat = typer.Option(OutputFormat.markdown, "--format", "-f", help="Output format"),
    theme: TemplateTheme = typer.Option(TemplateTheme.professional, "--theme", "-t", help="CV template theme"),
    ...
    try:
        ...
        if format in [OutputFormat.pdf, OutputFormat.all] and not is_pdf_available():
            if format == OutputFormat.pdf:
                typer.echo("⚠️  PDF generation is not available on this system.", err=True)
                typer.echo("🔄 Switching to HTML format instead...", err=True)
                format = OutputFormat.html
            else:  # format == OutputFormat.all
                typer.echo("⚠️  PDF generation is not available. Will generate Markdown and HTML only.", err=True)
        ...
        if verbose:
            ...
            typer.echo(f"🏆 Total stars: {data['contribution_patterns']['total_stars_earned']}")
        ...
        if custom_filename:
            ...
            base_name = custom_filename.rsplit('.', 1)[0] if '.' in custom_filename else custom_filename
            if format == OutputFormat.all:
                ...
                if format == OutputFormat.markdown and custom_filename.endswith(('.pdf', '.html')):
                    # Auto-detect format from extension
                    detected_format = custom_filename.split('.')[-1].lower()
                    if detected_format == 'pdf' and not is_pdf_available():
                        typer.echo("⚠️  PDF generation not available. Creating HTML file instead.", err=True)
                        ext = "html"
                        format = OutputFormat.html
                    else:
                        ext = detected_format
            ...
            try:
                if output_format == "markdown":
                    ...
                    if is_pdf_available():
                        ...
                        typer.echo(f"⚠️  Skipping PDF generation for {output_path} (not available)", err=True)
                        continue
                ...
                typer.echo(f"❌ Failed to generate {output_format.upper()}: {str(e)}", err=True)
                continue
...
def config():
    ...
    if not pdf_available:
        ...
        typer.echo("    • sudo apt-get install libpango1.0-dev libharfbuzz-dev libffi-dev")
        typer.echo("  macOS:")
        typer.echo("    • brew install pango harfbuzz")

@app.command()
def clean(
    username: Optional[str] = typer.Argument(None, help="Username to clean (or all if not specified)"),
...
def doctor():
    ...
    if output_dir.exists():
        ...
        total_size = sum(f.stat().st_size for folder in folders for f in folder.rglob('*') if f.is_file())
        typer.echo(f"📁 Generated CVs: {len(folders)} folders ({total_size / 1024:.1f}KB total)")
    else:
