...
def create_test_firecrawl_wrapper():
    ...
    def test_firecrawl_scrape(url: str, formats=None, onlyMainContent=True, waitFor=3000, extract=None):
        """
...
""",
                    "extract": {
                        ...
                        "skills": ["Web Development", "Mobile Development", "Backend Development", "Database Design", "Cloud Computing"],
                        "experience": [
                            "Software Developer at Tech Company",
                            "Full Stack Developer at Digital Agency", 
                            "Freelance Developer"
                        ...
                        "services": ["Web Development", "Mobile Development", "Consulting"],
            ...
            elif any(domain in url for domain in [".dev", ".tech", ".io"]) and "github" not in url:
...
""",
                    "extract": {
                        ...
                        "skills": ["JavaScript", "TypeScript", "Python", "React", "Node.js", "AWS", "Azure"],
                        "experience": ["Software Architect", "Technical Consultant"],
                        "education": [],
                        "projects": ["Cloud migration projects", "Enterprise applications"],
                        "services": ["Software Architecture", "Technical Consulting", "Code Reviews", "Training"],
                        "contact": {"website": url},
                        "social": {},
...
def analyze_profile_websites(username: str, verbose: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if verbose:
            print(f"📊 Fetching GitHub profile...")
        ...
        if verbose:
            ...
            if basic_info['bio']:
                print(f"   📝 Bio: {basic_info['bio'][:80]}{'...' if len(basic_info['bio']) > 80 else ''}")
        ...
        if verbose:
            ...
            if discovered_urls:
                ...
                print(f"   ❌ No personal websites discovered")
        ...
        if discovered_urls:
            ...
            if enrichment:
                ...
                enrichment_result = {
                    ...
                    "experience_entries": len(insights.get('additional_experience', [])),
                ...
                if verbose:
                    ...
                    print(f"      Websites crawled: {enrichment_result['websites_crawled']}")
                    print(f"      Additional skills: {enrichment_result['additional_skills']}")
                    print(f"      Experience entries: {enrichment_result['experience_entries']}")
                    if enrichment_result['top_skills']:
                        print(f"      Top skills: {', '.join(enrichment_result['top_skills'])}")
...
def generate_enhanced_cv_for_profile(analysis_result: Dict[str, Any], output_format: str = "markdown") -> bool:
    """
        ...
        if not analysis.get("error"):
            ...
            if analysis.get("discovered_urls"):
                ...
                if analysis.get("enrichment"):
                    ...
                    summary_stats["total_additional_skills"] += analysis["enrichment"].get("additional_skills", 0)
...
def print_comprehensive_summary(results: Dict[str, Any], stats: Dict[str, Any]):
    ...
    if stats['total_profiles'] > 0:
        ...
        website_rate = (stats['profiles_with_websites'] / stats['successful_analyses']) * 100 if stats['successful_analyses'] > 0 else 0
        enrichment_rate = (stats['profiles_with_enrichment'] / stats['profiles_with_websites']) * 100 if stats['profiles_with_websites'] > 0 else 0
    ...
    for username, result in results.items():
        ...
        if urls:
            ...
            print(f"      Discovered URLs: None")
        
        enrichment = result.get("enrichment")
        if enrichment:
            print(f"      Enrichment: ✅ Success")
            print(f"         Additional skills: {enrichment.get('additional_skills', 0)}")
            print(f"         Experience entries: {enrichment.get('experience_entries', 0)}")
            if enrichment.get('top_skills'):
                print(f"         Top skills: {', '.join(enrichment['top_skills'])}")
        else:
            print(f"      Enrichment: ❌ No enrichment data")
    
    # Feature flexibility demonstration
    print(f"\n🔧 Feature Flexibility Demonstrated:")
    print(f"   ✅ Handles profiles with personal websites")
    print(f"   ✅ Handles profiles without websites gracefully") 
    print(f"   ✅ Validates and filters discovered URLs")
    print(f"   ✅ Provides fallback behavior when enrichment fails")
    print(f"   ✅ Maintains consistent CV generation regardless of enrichment status")
    print(f"   ✅ Preserves original profile data when no websites found")
...
def main():
    ...
    print(f"🎉 Website enrichment system demonstrates excellent flexibility!")
