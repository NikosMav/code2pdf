...
class TestLanguageAnalysis:
    ...
    def test_expertise_levels(self):
        ...
        repos = [
            ...
            {'language': 'Python', 'stars': 10, 'size': 1000},  # 5 repos with good stars = Expert
            {'language': 'JavaScript', 'stars': 2, 'size': 200},  # Low stars, few repos = Beginner
        ]
        result = analyze_languages(repos)
...
class TestContributionPatterns:
    ...
    def test_basic_patterns(self):
        ...
        repos = [
            {
                'stars': 20, 'forks': 5, 'watchers': 15, 'open_issues': 3,
                'days_since_update': 30, 'has_wiki': True, 'description': 'Test repo',
                'license': 'MIT'
            },
            {
                'stars': 10, 'forks': 2, 'watchers': 8, 'open_issues': 1,
                'days_since_update': 60, 'has_wiki': False, 'description': None,
                'license': None
            }
        ]
    ...
    def test_impact_scores(self):
        ...
        # High impact
        high_impact_repos = [{'stars': 150, 'forks': 30, 'watchers': 100, 'open_issues': 5, 'days_since_update': 10}]
        result = analyze_contribution_patterns(high_impact_repos, {})
        assert result['impact_score'] == 'High'
        
        # Medium impact
        medium_impact_repos = [{'stars': 50, 'forks': 10, 'watchers': 30, 'open_issues': 2, 'days_since_update': 20}]
        result = analyze_contribution_patterns(medium_impact_repos, {})
        assert result['impact_score'] == 'Medium'
        
        # Low impact
        low_impact_repos = [{'stars': 5, 'forks': 1, 'watchers': 3, 'open_issues': 0, 'days_since_update': 100}]
